...
function CurrentlyOccupiedContent() {
  ...
  const load = async (currentPage = 1, sortField = 'bed_number', sortOrder = 'asc') => {
    setLoading(true); setError("");
    try {
      const response = await fetchCurrentlyOccupied({ 
        page: currentPage, 
        limit: itemsPerPage, 
        sortField, 
        sortOrder,
  ...
  // Initial load and reload when page/sort changes
  useEffect(() => { 
    load(page, sortConfig.key, sortConfig.direction); 
  }, [page, sortConfig]);
  ...
  const downloadCSV = () => {
    ...
    link.download = `currently_occupied_${new Date().toISOString().slice(0,10)}.csv`;
    link.click();
  };
  ...
  const sendCSVEmail = async () => {
    ...
    setSendingCSV(true);
    try {
      const response = await fetch(`${API_BASE_URL}/api/allocations/currently-occupied/send-csv`, {
        method: 'POST',
        headers: {
    ...
    <div className="p-3 sm:p-4 max-w-[1800px] mx-auto">
      ...
      <section className="relative overflow-hidden rounded-xl sm:rounded-2xl bg-gradient-to-br from-indigo-900/20 via-purple-900/20 to-blue-900/20 border border-indigo-500/20 p-4 sm:p-6 mb-4 sm:mb-6">
        <div className="relative">
          ...
          <p className="text-sm sm:text-base text-indigo-200/80">
            View all beds currently occupied
          </p>
        </div>
      </section>
      ...
      <section className="rounded-xl border border-gray-700 bg-gray-900/50 backdrop-blur p-4 mb-4">
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-4">
          ...
          <div>
            <label className="block text-sm font-medium text-gray-300 mb-2">Tent</label>
            <select
              value={filters.tent_index}
              onChange={(e) => setFilters(prev => ({ ...prev, tent_index: e.target.value }))}
              disabled={!filters.location_id}
              className="w-full bg-gray-800/50 border border-gray-600 rounded-lg px-3 py-2 text-gray-100 focus:outline-none focus:ring-2 focus:ring-blue-500 disabled:bg-gray-700/30 disabled:cursor-not-allowed disabled:text-gray-500"
            >
              <option value="">All Tents</option>
              {tents.map(tent => (
                <option key={tent.index} value={tent.index}>{tent.name}</option>
              ))}
            </select>
          </div>
          ...
          <div>
            ...
            >
              ...
              {blocks.map(block => (
                <option key={block.index} value={block.index}>{block.name}</option>
              ))}
            </select>
          </div>
        </div>
        ...
        <div className="mt-4">
          ...
          <button
            onClick={() => {
              setPage(1);
              load(1, sortConfig.key, sortConfig.direction);
            }}
            className="px-6 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg text-sm font-medium transition-colors w-full sm:w-auto"
          >
            Apply Filters
          </button>
        </div>
